...
18. ✅ **Skills migration (optional)** — EVALUATED 2026-06-12, verdict: **defer**. The concrete benefit skills offer (bundled supporting files) is already achieved — the `/tdd` pattern catalog lives at `docs/testing-patterns.md` and is referenced by path from three commands. Migration would fragment the flat `commands/*.md` layout the Quick Install copy flow depends on. Revisit only if a command needs per-skill supporting files that can't live in `docs/`. **Superseded 2026-07-26 for the research workflow family only:** conventions v0.2 (`docs/conventions.md` §1) adopt a skill-package kernel for `/research_codebase`; the pilot's rewrite PR updates Quick Install to also copy `skills/`, keeping non-plugin installs complete (see `2026-07-26-thinking-model-modernization-pilot.md`, Packaging). All other command families remain deferred pending their own gates.
