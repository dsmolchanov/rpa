...
Capability → proof map (each row is backed by named preflight checks; 68 checks total):
...
- schedule: `runner.py --config C --make-schedule --tasks T1.md T2.md --replicates 3 --seed S --schedule-out schedule.json` (non-holdout shapes need `--allow-nonstandard`), then `runner.py --config C --run-schedule schedule.json --repo /path/to/clone --tasks T1.md T2.md` (the same registered task set, revalidated)
