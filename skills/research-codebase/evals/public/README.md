...
Capability → proof map (each row is backed by named preflight checks; 52 checks total):
...
| Pre-registered run schedule | `--make-schedule` emits a balanced, seed-recorded randomized interleaving of arm × task × replicate (per-arm `schedule_tasks` carries the pre-registered third-arm scoping); `--run-schedule` executes it in recorded order, refuses tampered/unbalanced/reordered schedule files, and writes a completion manifest |
...
- schedule: `runner.py --config C --make-schedule --tasks T1.md T2.md --replicates 3 --seed S --schedule-out schedule.json`, then `runner.py --config C --run-schedule schedule.json --repo /path/to/clone`
