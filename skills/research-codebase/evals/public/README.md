...
Capability → proof map (each row is backed by named preflight checks; 57 checks total):
...
| Pinned backend version | the exact Claude Code version is registered (`backend_version` + `backend_version_cmd`) and probed before every run and every judge pass; version drift between interleaved schedule entries blocks the run |
