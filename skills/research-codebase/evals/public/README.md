...
Capability → proof map (each row is backed by named preflight checks; 30 checks total):
...
| Per-node model/effort capture | transcript nodes carry `model` and `effort`; effort must be pinned via the `{effort}` placeholder in `backend_cmd` (a command without it is refused); `validate_models` / `validate_efforts` invalidate runs whose nodes mismatch — or omit — the registered values |
