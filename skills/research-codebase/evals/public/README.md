...
Capability → proof map (each row is backed by named preflight checks; 43 checks total):

| Capability (plan, prereq 5) | Runner mechanism |
|---|---|
| Installation-hash verification | `hash_tree` + `verify_installation` for **every registered arm** before every run (drift in any arm halts the experiment); the selected arm's verified copy is **mounted into the profile** and its path passed to the backend via the `{installation}` placeholder in `backend_cmd` |
