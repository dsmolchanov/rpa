...
Pattern examples (unit/integration/E2E shapes, mocking at boundaries, common mistakes, test organization) live in the shared reference: **`docs/testing-patterns.md`** in this plugin. Consult it when writing tests in the RED phase; do not reinvent structures it already defines. For per-language generation templates (Jest, pytest, Go) and snapshot sanitization, the `test-generator` agent is authoritative.

Key rules carried over (non-negotiable):
- Test user-visible behavior, not implementation internals
- Semantic selectors only (`data-testid`, roles, text) — never CSS-hash selectors
- Each test independent: own setup, no shared mutable state
- No arbitrary waits — wait for conditions
